from logging import *
from configuration import Configuration

class Users(Configuration):
	FILENAME = "users.ini"
	
	PASSWORD = "password"
	BANKROLL = "bankroll"
	STARTING_BET = "starting_bet"
	
	def __init__(self, log):
		# Initialize and read INI file
		Configuration.__init__(self, log, self.FILENAME)

	def get_bankroll(self, user):
		bankroll = 0
		if user in self.config:
			try:
				bankroll = float(self.config[user][self.BANKROLL])
			except ValueError:
				pass
		return bankroll

	def get_starting_bet(self, user):
		starting_bet = 0
		if user in self.config:
			try:
				starting_bet = int(self.config[user][self.STARTING_BET])
			except ValueError:
				pass
		return starting_bet

	def login(self, user, password):
		success = False
		if user in self.config and self.config[user][self.PASSWORD] == password:
			success = True
		return success

	def has_user(self, user):
		has = False
		if user in self.config:
			has = True
		return has

	def save(self, user, bankroll, starting_bet):
		if not user in self.config:
			self.config[user] = {}
		self.config[user][self.BANKROLL] = str(bankroll)
		self.config[user][self.STARTING_BET] = str(starting_bet)
		with open(self.FILENAME, 'w') as configfile:
			self.config.write(configfile)

	def save_new_user(self, user, password, bankroll, starting_bet):
		if not user in self.config:
			self.config[user] = {}
		self.config[user][self.PASSWORD] = password
		self.save(user, bankroll, starting_bet)
	